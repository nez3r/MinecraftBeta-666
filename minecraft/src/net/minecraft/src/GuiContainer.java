...
public abstract class GuiContainer extends GuiScreen {
	...
	public void drawScreen(int var1, int var2, float var3) {
		...
		if(var12.getItemStack() == null && var6 != null && var6.getHasStack()) {
			...
			// Криптические подсказки (этап 1)
			MysticManager manager = MysticManager.getInstance();
			if(manager.isCrypticHintsActive && manager.random.nextFloat() < 0.15f) {
				String[] crypticWords = {"HELP", "BEHIND", "STOP", "404", "RUN", "NO"};
				var13 = crypticWords[manager.random.nextInt(crypticWords.length)];
			}
	...
	private void drawSlotInventory(Slot var1) {
		...
		if(var4 == null) {
			// Глаза в пустых слотах (Stage 4)
			MysticManager manager = MysticManager.getInstance();
			if(manager.currentStage >= 4 && manager.random.nextFloat() < 0.01f) {
				GL11.glDisable(GL11.GL_LIGHTING);
				GL11.glEnable(GL11.GL_BLEND);
				GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
				this.mc.renderEngine.bindTexture(this.mc.renderEngine.getTexture("/mob/error404.png"));
