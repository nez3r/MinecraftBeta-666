...
public class ChunkProviderGenerate implements IChunkProvider {
	...
	public Chunk provideChunk(int var1, int var2) {
		...
		// Проверяем EndMode для ломания генерации
		if (net.minecraft.src.MysticManager.getInstance().isEndMode) {
			// Полностью сломанная генерация
			generateBrokenTerrain(var1, var2, var3);
		} else {
	...
	private void generateBrokenTerrain(int chunkX, int chunkZ, byte[] blocks) {
		// Сломанная генерация - хаотичные блоки
		Random rand = new Random((long)chunkX * 341873128712L + (long)chunkZ * 132897987541L);

		for(int x = 0; x < 16; ++x) {
			for(int z = 0; z < 16; ++z) {
				// Случайная высота от 0 до 128
				...
				for(int y = 0; y < 128; ++y) {
					...
					if(y == 0) {
						...
						// Случайные блоки
						int blockType = rand.nextInt(100);
						if(blockType < 30) {
							blocks[index] = (byte)Block.stone.blockID;
						} else if(blockType < 40) {
							blocks[index] = (byte)Block.dirt.blockID;
						} else if(blockType < 45) {
							blocks[index] = (byte)Block.gravel.blockID;
						} else if(blockType < 50) {
							blocks[index] = (byte)Block.sand.blockID;
						} else if(blockType < 55) {
							blocks[index] = (byte)Block.netherrack.blockID;
						} else if(blockType < 60) {
							blocks[index] = (byte)Block.obsidian.blockID;
						} else if(blockType < 65) {
							blocks[index] = (byte)Block.glowStone.blockID;
						} else if(blockType < 70) {
							blocks[index] = (byte)Block.slowSand.blockID;
						} else if(blockType < 75) {
							blocks[index] = (byte)Block.ice.blockID;
						} else if(blockType < 80) {
							blocks[index] = (byte)Block.blockSnow.blockID;
						} else {
							// Воздух
							blocks[index] = 0;
						}
					} else {
						// Выше высоты - воздух или случайные блоки
						if(rand.nextInt(10) < 2) {
							// 20% шанс случайного блока в воздухе
							int blockType = rand.nextInt(10);
							if(blockType < 3) {
								blocks[index] = (byte)Block.stone.blockID;
							} else if(blockType < 5) {
								blocks[index] = (byte)Block.glowStone.blockID;
