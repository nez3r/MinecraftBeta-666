	...
	public void renderString(String var1, int var2, int var3, int var4, boolean var5) {
		if(var1 != null) {
			...
			for(var6 = 0; var6 < var1.length(); ++var6) {
				...
				if(var6 < var1.length()) {
					char currentChar = var1.charAt(var6);

					// Глитч текста в End Mode - заменяем буквы на цифры
					if(MysticManager.getInstance().isEndMode && Character.isLetter(currentChar)) {
						currentChar = (char)('0' + new java.util.Random().nextInt(10));
					}

					var11 = ChatAllowedCharacters.allowedCharacters.indexOf(currentChar);
					if(var11 >= 0) {
