	...
	public void renderGameOverlay(float var1, boolean var2, int var3, int var4) {
		...
		if(this.mc.playerController.shouldDrawHUD()) {
			...
			for(var16 = 0; var16 < 10; ++var16) {
				...
				// GUI Shaking - тряска интерфейса при активном событии
				int shakeX = 0;
				int shakeY = 0;
				if(MysticManager.getInstance().isGuiShakingActive) {
					shakeX = this.rand.nextInt(3) - 1;
					shakeY = this.rand.nextInt(3) - 1;
				}

				if(var15 > 0) {
					var18 = var6 / 2 + 91 - var16 * 8 - 9;
					if(var16 * 2 + 1 < var15) {
						this.drawTexturedModalRect(var18 + shakeX, var17 + shakeY, 34, 9, 9, 9);
					}

					if(var16 * 2 + 1 == var15) {
						this.drawTexturedModalRect(var18 + shakeX, var17 + shakeY, 25, 9, 9, 9);
					}

					if(var16 * 2 + 1 > var15) {
						this.drawTexturedModalRect(var18 + shakeX, var17 + shakeY, 16, 9, 9, 9);
					}
				}
				...
				this.drawTexturedModalRect(var19 + shakeX, var17 + shakeY, 16 + var28 * 9, 0, 9, 9);
				if(var12) {
					if(var16 * 2 + 1 < var14) {
						this.drawTexturedModalRect(var19 + shakeX, var17 + shakeY, 70, 0, 9, 9);
					}

					if(var16 * 2 + 1 == var14) {
						this.drawTexturedModalRect(var19 + shakeX, var17 + shakeY, 79, 0, 9, 9);
					}
				}

				if(var16 * 2 + 1 < var13) {
					this.drawTexturedModalRect(var19 + shakeX, var17 + shakeY, 52, 0, 9, 9);
				}

				if(var16 * 2 + 1 == var13) {
					this.drawTexturedModalRect(var19 + shakeX, var17 + shakeY, 61, 0, 9, 9);
				}
			}
		...
		if(this.mc.gameSettings.showDebugInfo) {
			...
			// Глитч названия игры в F3
			String gameTitle = "Minecraft Beta 1.6.6";
			if(MysticManager.getInstance().isEndMode || MysticManager.getInstance().isGlitchMode) {
				gameTitle = MysticManager.getInstance().generateGlitchedString(gameTitle);
			}

			var8.drawStringWithShadow(gameTitle + " (" + this.mc.debug + ")", 2, 2, 16777215);
			...
			// Entity detector - fake entity count
			if (MysticManager.getInstance().isFakeEntityDetectorActive) {
				int fakeCount = 1 + MysticManager.getInstance().random.nextInt(3);
				this.drawString(var8, "\u00A7c[!] Unknown entities detected: " + fakeCount, 2, 88, 16733525);
			}
