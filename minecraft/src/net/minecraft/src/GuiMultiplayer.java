	...
	protected void actionPerformed(GuiButton var1) {
		if(var1.enabled) {
			if(var1.id == 1) {
				...
				// Проверка на 404 при нажатии Connect
				if(var2.equals("404")) {
					// Воспроизводим звук jumpscare
					try {
					...
					// Активируем инверсию экрана (используем флаг без таймера для меню)
					MysticManager manager = MysticManager.getInstance();
					manager.isInversionActive = true;

					// Активируем интенсивную тряску окна через MysticManager
					manager.trigger404Screamer();
