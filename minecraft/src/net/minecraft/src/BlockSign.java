...
public class BlockSign extends BlockContainer {
	...
	public boolean blockActivated(World var1, int var2, int var3, int var4, EntityPlayer var5) {
		// Проверка на табличку с текстом "404"
		TileEntity tileEntity = var1.getBlockTileEntity(var2, var3, var4);
		if(tileEntity instanceof TileEntitySign) {
			TileEntitySign sign = (TileEntitySign)tileEntity;
			// Проверяем все строки на наличие "404"
			for(int i = 0; i < sign.signText.length; i++) {
				if(sign.signText[i] != null && sign.signText[i].equals("404")) {
					// Активируем быстрое моргание инверсии на 5 секунд
					MysticManager manager = MysticManager.getInstance();
					manager.triggerSignInversion();

					// Случайное сообщение
					String[] messages = {"Hi!", "What?", "Stop it.", "Leave me alone.", "404", "Why?"};
					var5.addChatMessage(messages[new java.util.Random().nextInt(messages.length)]);
