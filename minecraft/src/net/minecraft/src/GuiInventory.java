...
public class GuiInventory extends GuiContainer {
	...
	protected void drawGuiContainerBackgroundLayer(float var1) {
		...
		// Infinite inventory effect (Stage 4) - duplicate inventory multiple times
		MysticManager manager = MysticManager.getInstance();
		if (manager.isInfiniteInventoryActive) {
			long time = System.currentTimeMillis();
			for (int i = 0; i < 8; i++) {
				int offsetX = (int)(Math.sin(time / 200.0 + i) * 50);
				int offsetY = (int)(Math.cos(time / 200.0 + i) * 50);
				this.drawTexturedModalRect(var3 + offsetX, var4 + offsetY, 0, 0, this.xSize, this.ySize);
			}
		} else {
