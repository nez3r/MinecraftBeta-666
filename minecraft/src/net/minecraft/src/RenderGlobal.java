...
public class RenderGlobal implements IWorldAccess {
	...
	public String getDebugInfoEntities() {
		// Фейковый детектор сущностей (Этап 3)
		int fakeEntities = this.countEntitiesTotal;
		if (MysticManager.getInstance().isFakeEntityDetectorActive) {
			// Добавляем от 1 до 5 фейковых сущностей
			fakeEntities += 1 + MysticManager.getInstance().random.nextInt(5);
		}
		return "E: " + this.countEntitiesRendered + "/" + fakeEntities + ". B: " + this.countEntitiesHidden + ", I: " + (fakeEntities - this.countEntitiesHidden - this.countEntitiesRendered);
	}
	...
	public void renderSky(float var1) {
		if(!this.mc.theWorld.worldProvider.isNether) {
			...
			// Sky glitch effect (Stage 4) - use terrain texture instead of sun/moon
			MysticManager manager = MysticManager.getInstance();
			if (manager.isSkyGlitchActive) {
	...
	public void renderEyesInFog(float var1) {
		MysticManager manager = MysticManager.getInstance();
		if (!manager.isEyesInFogActive) {
			return;
		}
		...
		double playerX = player.lastTickPosX + (player.posX - player.lastTickPosX) * (double)var1;
		double playerY = player.lastTickPosY + (player.posY - player.lastTickPosY) * (double)var1;
		double playerZ = player.lastTickPosZ + (player.posZ - player.lastTickPosZ) * (double)var1;
		...
		for (int i = 0; i < 8; i++) {
			...
			double distance = 15.0 + rand.nextDouble() * 10.0;
			double height = rand.nextDouble() * 4.0 - 2.0;

			double eyeX = playerX + Math.cos(angle) * distance;
			double eyeY = playerY + height + 1.6;
			double eyeZ = playerZ + Math.sin(angle) * distance;

			GL11.glPushMatrix();
			GL11.glTranslated(eyeX - playerX, eyeY - playerY, eyeZ - playerZ);

			float alpha = (float)(Math.sin(System.currentTimeMillis() / 500.0 + i) * 0.3 + 0.5);
			GL11.glColor4f(1.0F, 0.0F, 0.0F, alpha);

			// Left eye
			tess.startDrawingQuads();
			tess.addVertex(-0.15, -0.1, 0.0);
			tess.addVertex(-0.05, -0.1, 0.0);
			tess.addVertex(-0.05, 0.1, 0.0);
			tess.addVertex(-0.15, 0.1, 0.0);
			tess.draw();
