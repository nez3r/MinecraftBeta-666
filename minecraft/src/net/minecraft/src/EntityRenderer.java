...
public class EntityRenderer {
	...
	private void orientCamera(float var1) {
		...
		// World mirror effect (Stage 4) - flip vertically
		MysticManager manager = MysticManager.getInstance();
		if (manager.isWorldMirrorActive) {
	...
	private void setupCameraTransform(float var1, int var2) {
		...
		// Жесткий глитч эффект в END режиме
		if(MysticManager.getInstance().isEndMode) {
			// Быстрое мерцание дальности прорисовки
			this.farPlaneDistance = (float)(10 + new java.util.Random().nextInt(100));
		}
	...
	public void updateCameraAndRender(float var1) {
		...
		if(!this.mc.skipRenderWorld) {
			...
			if(this.mc.theWorld != null) {
				...
				// Применяем визуальные эффекты мистики после рендеринга мира
				applyMysticVisualEffects();
			} else {
			...
			// Применяем визуальные эффекты мистики ВСЕГДА (в конце рендеринга)
			applyMysticVisualEffects();
	...
	public void renderWorld(float var1, long var2) {
		...
		for(int var18 = 0; var18 < 2; ++var18) {
			...
			if(!this.mc.gameSettings.anaglyph) {
				// Применяем визуальные эффекты мистики
				applyMysticVisualEffects();
				return;
			}
		}
	...
	private void applyMysticVisualEffects() {
		MysticManager manager = MysticManager.getInstance();

		// Проверяем инверсию от таблички (обновляет состояние)
		manager.isSignInversionActive();

		// Инверсия экрана
		if(manager.isInversionActive) {
			GL11.glMatrixMode(GL11.GL_PROJECTION);
			GL11.glPushMatrix();
			GL11.glLoadIdentity();
			...
			tess.addVertex(0.0D, (double)this.mc.displayHeight, 0.0D);
			tess.addVertex((double)this.mc.displayWidth, (double)this.mc.displayHeight, 0.0D);
			tess.addVertex((double)this.mc.displayWidth, 0.0D, 0.0D);
		...
		if(manager.isVHSActive) {
			...
			for(int i = 0; i < 20; i++) {
				int y = rand.nextInt(this.mc.displayHeight);
				int height = rand.nextInt(5) + 1;
				int alpha = rand.nextInt(60) + 30; // Затемнено: было 100+50, стало 60+30
				tess.startDrawingQuads();
				tess.setColorRGBA(200, 200, 200, alpha); // Затемнено: было 255,255,255, стало 200,200,200
				tess.addVertex(0.0D, (double)(y + height), 0.0D);
				tess.addVertex((double)this.mc.displayWidth, (double)(y + height), 0.0D);
				tess.addVertex((double)this.mc.displayWidth, (double)y, 0.0D);
				tess.addVertex(0.0D, (double)y, 0.0D);
				tess.draw();
			}
		...
		// Зеркальный мир (вертикальная инверсия)
		if(manager.isWorldMirrorActive) {
			GL11.glScalef(1.0F, -1.0F, 1.0F);
		}
	}
	...
	private void setupFog(int var1, float var2) {
		...
		if(this.cloudFog) {
			...
			// Fog collapse effect (Stage 4)
			MysticManager manager = MysticManager.getInstance();
			if (manager.isFogCollapseActive) {
				GL11.glFogf(GL11.GL_FOG_START, 0.0F);
				GL11.glFogf(GL11.GL_FOG_END, 5.0F); // Very close fog
			} else {
